...
constexpr const char* kProviderSignature =
    "UCM_ASU_AICPU_PROVIDER_UBC_CTP_UBG_HCOMM_HIXL_V10";
...
public:
    explicit AclContextScope(const char* stage) : stage_(stage)
    {
        ...
        if (ret != ACL_SUCCESS || savedContext_ == nullptr) {
            UC_DEBUG("AICPUProvider: no ACL context to preserve stage={} "
        ...
        UC_DEBUG("AICPUProvider: preserved caller ACL context stage={} context={} "
    ...
    ~AclContextScope()
    {
        ...
        if (setRet != ACL_SUCCESS) {
            ...
            UC_ERROR("AICPUProvider: failed to restore caller ACL context stage={} "
        ...
        UC_DEBUG("AICPUProvider: restored caller ACL context stage={} context={} "
...
LocalDeviceSelection ResolveLocalDevice(const TransportConfig& config)
{
    ...
    if (deviceRet != ACL_SUCCESS || currentDevice < 0) {
        UC_WARN("AICPUProvider: current ACL logical device unavailable ret={} device_id={}; "
    ...
    if (contextRet != ACL_SUCCESS) {
        UC_WARN("AICPUProvider: current ACL context unavailable for logical_device_id={} "
...
Status ResolveProtocol(const TransportConfig& config, const AsuEndpoint* endpoint,
                       CommProtocol& protocol)
{
...
Status FillCommAddr(const std::string& text, CommAddr& out)
{
    if (TryFillEidCommAddr(text, out)) {
        UC_INFO("AICPUProvider: parsed HCOMM endpoint address as EID addr={}", text);
        return Status::OK();
    }
                         ...
                         const char* role, EndpointDesc& out)
{
    ...
    UC_DEBUG("AICPUProvider: EndpointDesc resolved role={} addr={} addr_type={} "
             "device_id={} protocol={}",
...
struct AICPUProvider::Impl {
    ...
    Status RefreshLocalDeviceFromCaller()
    {
        ...
        if (deviceRet != ACL_SUCCESS || currentDevice < 0) {
            ...
            UC_WARN("AICPUProvider: caller ACL device unavailable before "
        ...
        const auto contextRet = aclrtGetCurrentContext(&currentContext);
        if (contextRet != ACL_SUCCESS) {
            UC_WARN("AICPUProvider: cannot capture caller ACL context before "
        ...
        UC_INFO("AICPUProvider: refreshed Provider ACL binding at CreateConnection "
    ...
    Status EnsureEndpointLocked(const std::string& localIp, CommProtocol protocol)
    {
        if (endpoint != nullptr) {
            if (endpointProtocol != protocol) {
                UC_ERROR("AICPUProvider: mixed hcomm protocols are not supported "
            ...
            UC_DEBUG("AICPUProvider: reusing HCOMM endpoint local_addr={} protocol={}",
        ...
        if (resolvedLocalIp.empty()) {
            ...
            UC_ERROR("AICPUProvider: local {} is required for hcomm endpoint "
        ...
        UC_INFO("AICPUProvider: creating HCOMM endpoint local_addr={} address_source={} "
        ...
        auto status = BuildEndpointDesc(config, nullptr, resolvedLocalIp, localDeviceId, protocol,
                                        "local", localDesc);
        if (!status.ok()) {
            UC_ERROR("AICPUProvider: local EndpointDesc build failed local_addr={} "
        ...
        UC_INFO("AICPUProvider: HCOMM endpoint created local_addr={} local_device_id={} "
                "protocol={}",
    ...
    Status EnsureAclDeviceBound(const char* stage)
    {
        ...
        if (providerContext != nullptr) {
            ...
            UC_INFO("AICPUProvider: bound captured ACL context stage={} context={} "
        ...
        UC_INFO("AICPUProvider: aclrtSetDevice bound logical_device_id={} stage={} "
...
#if UCM_ASU_AICPU_USE_STAGED_CHANNEL_API
        UC_INFO("AICPUProvider: HcommChannelUpdateStagedLocalMemInfo begin "
...
#else
        UC_INFO("AICPUProvider: HcommChannelUpdateMemInfo begin tag={} channel={}",
        ...
        const std::vector<StagedPublishTarget>& additionalTargets = {})
    {
        ...
        targets.erase(
            ...
            targets.end());
        if (targets.empty()) {
            UC_DEBUG("AICPUProvider: defer staged MR publication tag={} until a "
        ...
        for (const auto& target : targets) {
            UC_INFO("AICPUProvider: publishing staged MR tag={} mr_id={} original_addr={} "
        ...
        UC_INFO("AICPUProvider: staged MR published tag={} mr_id={} original_addr={} "
                              ...
                              std::uintptr_t& mappingBase, std::uintptr_t& deviceAddr)
    {
        ...
        if (iter != hostMappings.end()) {
            ...
            UC_INFO("AICPUProvider: reused host mapping host_addr={} mapping_base={} "
        ...
        UC_INFO("AICPUProvider: created host mapping host_addr={} mapping_base={} "
    ...
    void ReleaseHostMapping(std::uintptr_t mappingBase)
    {
        ...
        auto iter = hostMappings.find(mappingBase);
        if (iter == hostMappings.end()) {
            UC_WARN("AICPUProvider: host mapping release missed mapping_base={}",
                    mappingBase);
            return;
        }
        if (--iter->second.refCount != 0) {
            UC_INFO("AICPUProvider: retained host mapping mapping_base={} mapped_base={} "
                    "ref_count={}",
        ...
        UC_INFO("AICPUProvider: released host mapping mapping_base={} mapped_base={} "
                "mapping_size={}",
                                 ...
                                 std::vector<std::uint32_t>& hixlStatuses)
    {
        ...
        for (const auto& io : ioBatches) {
            ...
            UC_DEBUG("AICPUProvider: batch send entry channel={} thread={} addr={} len={} imm=0x{:x}",
                                       ...
                                       std::vector<ConnectionHandle>& connectionHandles)
{
    ...
    UC_INFO("AICPUProvider: CreateConnection start signature={} pid={} local_addr={} "
    ...
    status = BuildEndpointDesc(impl_->config, endpoint, remoteIp, remoteDeviceId, protocol,
                               "remote", remoteDesc);
    if (!status.ok()) {
        UC_ERROR("AICPUProvider: remote EndpointDesc build failed remote_addr={} "
...

    {
        ...
        if (!status.ok()) {
            UC_ERROR("AICPUProvider: EnsureEndpointLocked failed local_addr={} "
                     "local_device_id={} protocol={} message={}",
    ...
    UC_INFO("AICPUProvider: CreateConnection resolved mode=staged_{} channel_api={} "
    ...
    for (std::uint32_t remaining = qpNum; remaining > 0; --remaining) {
        ...
        UC_DEBUG("AICPUProvider: creating connection handle qp_index={} qp_num={} "
                 "channel_api={}",
        ...
        const auto cleanupRecord = [&]() {
            ...
            if (cleanupStatuses.empty() || !cleanupStatuses.front().ok()) {
                UC_WARN("AICPUProvider: retained partially initialized connection "
        ...
        UC_INFO("AICPUProvider: HcommChannelCreateStaged begin qp_index={} "
        ...
        UC_INFO("AICPUProvider: staged channel created qp_index={} channel={} thread={}",
        ...
        const auto capabilitiesInitRet =
            HcommStagedServerCapabilitiesInit(&hcommCapabilities, 1U);
        ...
        record->serverCapabilities.ioQueueKeyConcurrency =
            hcommCapabilities.ioQueueKeyConcurrency;
        ...
        UC_INFO("AICPUProvider: staged channel info qp_index={} controller_id={} "
        ...
        UC_INFO("AICPUProvider: staged server capabilities qp_index={} queue_num={} "
                ...
                qpIndex, record->serverCapabilities.queueNum,
                record->serverCapabilities.ioQueueDepth,
                record->serverCapabilities.ioQueueKeyConcurrency,
                record->serverCapabilities.connectionKeyConcurrency,
                record->serverCapabilities.singleValueMaxBytes,
                record->serverCapabilities.batchValueMaxBytes,
                record->serverCapabilities.batchStoreKeys,
                record->serverCapabilities.batchLoadKeys,
                record->serverCapabilities.deleteKeys, record->serverCapabilities.queryKeys,
                record->serverCapabilities.keyLength,
                record->serverCapabilities.kvCapabilities);
#else
        ...
        UC_INFO("AICPUProvider: HcommChannelCreate begin qp_index={} channel_port={}",
                qpIndex, desc.port);
        const auto chanRet =
        ...
        if (!status.ok()) {
            UC_ERROR("AICPUProvider: failed to attach existing memory to new channel "
                     "qp_index={} channel={} code={} message={}",
                     qpIndex, record->channel, static_cast<int>(status.code), status.message);
            cleanupRecord();
            if (!createdHandles.empty()) { (void)DeleteConnections(createdHandles); }
        ...
        UC_INFO("AICPUProvider: connection handle ready qp_index={} created_handles={}/{}",
                qpIndex, connectionHandles.size(), qpNum);
    }

    UC_INFO("AICPUProvider: CreateConnection complete remote_addr={} remote_port={} "
            "handles={}",
    ...
    const std::vector<ConnectionHandle>& connectionHandles)
{
    ...
    for (std::size_t index = 0; index < connectionHandles.size(); ++index) {
        ...
        if (status.ok()) {
            ...
            std::lock_guard<std::mutex> lock(impl_->mu);
            impl_->connections.insert(record);
            UC_WARN("AICPUProvider: retained connection handle={} channel={} thread={} "
                                        ...
                                        uint32_t kernelCount, uint32_t quietCount)
{
    ...
    for (const auto& group : groups) {
        UC_DEBUG("AICPUProvider: launching HixlBatchSend thread={} entries={}",
                                 ...
                                 std::vector<MRHandle>& mrHandles)
{
    ...
    for (const auto& region : regions) {
        if (region.memoryType != MemType::MEM_DEVICE) {
            UC_ERROR("AICPUProvider: BindMemory rejected non-device memory addr={} size={} "
                     "memory_type={}",
    ...
    const std::vector<std::uint32_t>* expectedTokenIds)
{
    ...
    UC_INFO("AICPUProvider: {} begin registration_mode={} desc_count={}", operation,
    ...
    for (const auto& desc : memoryDescs) {
        if (desc.addr == 0 || desc.size == 0) {
            UC_ERROR("AICPUProvider: {} invalid desc addr={} size={} type={}", operation,
        ...
        auto releaseCurrent = [&]() {
            ...
            UC_ERROR("AICPUProvider: retained mr_handle={} after rollback failure: {}",
        ...
        if (tokenRet == 0) {
            if (tokenInfo.addr != record->transportAddr || tokenInfo.size != record->size) {
                UC_ERROR("AICPUProvider: HCOMM token range mismatch tag={} "
            ...
            UC_WARN("AICPUProvider: HcommMemGetTokenInfo failed registration_mode={} tag={} "
        ...
        if (expectedTokenIds != nullptr) {
            ...
            if (!record->hasToken || record->tokenId != expectedToken) {
                UC_ERROR("AICPUProvider: {} token mismatch registration_mode={} tag={} "
        ...
        createdHandles.push_back(handle);
        mrHandles.push_back(handle);
        UC_INFO("AICPUProvider: {} record ready registration_mode={} mr_handle={} "
                "hcomm_mem_handle={} handles={}/{}",
                operation, registrationMode, handle, hcommMemHandle, mrHandles.size(),
                memoryDescs.size());
    }

    UC_INFO("AICPUProvider: {} complete registration_mode={} handles={}", operation,
                                                      ...
                                                      const char* operation)
{
    ...
    for (std::size_t reverseIndex = mrHandles.size(); reverseIndex > 0; --reverseIndex) {
        ...
        if (iter == impl_->memories.end()) {
            UC_DEBUG("AICPUProvider: {} ignored released handle={}", operation, handle);
            continue;
        }
